...
def fast_pass_annealing(ciphertext, locked_S, rotors, rings, pos):
    """
    A brutally fast sprint of Simulated Annealing designed to test the 
    'linguistic waters' of a specific Bombe offset. It quickly returns the peak score.
    """
    current_S = locked_S.copy()
    current_text = apply_decryption(ciphertext, current_S, rotors, rings, pos)
    best_score = fuzzy_language_score(current_text)
...
def generate_pdf_report(score_history, final_text, human_text, ciphertext, rotors, rings, pos, steckerbrett, offset):
    ...
    footer_text = "(c) by George W. Aravidis | Ulm Söflingen | Email : csec.unbutton326@passinbox.com | Historical Reference"
...
def run_ultimate_cracker():
 ...
 + --------------------------------------------------------- +
    ...
    if len(valid_bombe_results) > 1:
        print("\n+ " + "-"*57 + " +")
        print("| PHASE 1.5: META-HEURISTIC MULTI-OFFSET PRUNING          |")
        print("+ " + "-"*57 + " +")
        print(f"    [*] Found {len(valid_bombe_results)} conflicting valid offsets. Initiating fast-pass evaluation...")
        
        evaluated_results = []
        for idx, (ofs, b_S) in enumerate(valid_bombe_results):
            sys.stdout.write(f"\r    [PRUNING] Assessing offset {ofs:03d} out of {len(valid_bombe_results)}...\033[K")
            sys.stdout.flush()
            score = fast_pass_annealing(ciphertext, b_S, rotors, rings, pos)
            evaluated_results.append((score, ofs, b_S))
            
        evaluated_results.sort(key=lambda x: x[0], reverse=True)
        best_offset, bombe_S = evaluated_results[0][1], evaluated_results[0][2]
        print(f"\n    [+] Pruning complete. Offset {best_offset} selected as True Peak (Heuristic Score: {evaluated_results[0][0]}).")
    else:
        best_offset, bombe_S = valid_bombe_results[0]
        print(f"    [+] Bombe deduced initial Steckerbrett at unique offset {best_offset} in {time.time() - start_time:.4f} seconds.")
